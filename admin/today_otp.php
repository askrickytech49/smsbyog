...
function admin_api_source_name($api_id) {
  return [
    8 => 'Tiger SMS',
    2 => '5SIM',
    3 => 'USA + Canada',
...
$sql=mysqli_query($conn,"SELECT a.*,u.email FROM active_number a LEFT JOIN user_data u ON a.user_id=u.id WHERE $number_visibility AND a.status='1' ORDER BY a.id DESC");
...
<div class="admin-card">
  <div class="admin-card-body p-0">
    <div class="table-responsive admin-mobile-table-responsive">
      <table class="admin-table admin-mobile-table admin-datatable" style="width:100%" data-order='[[ 6, "desc" ]]'>
        <thead><tr><th>User</th><th>Number</th><th>Service</th><th>API Source</th><th>Price</th><th>OTP Code</th><th>Time</th></tr></thead>
        <tbody>
        ...
        <tr>
          ...
          <td data-label="API Source"><span class="badge bg-light text-dark"><?=htmlspecialchars(admin_api_source_name($r['api_id']??0))?></span></td>
          <td data-label="Price">₦<?=number_format($r['service_price']??0)?></td>
          <td data-label="OTP Code">
            <?php if($r['sms_text']): ?>
