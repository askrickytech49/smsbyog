            ...
            if ($left == "00:00" && $row['active_status'] == 2) {
                ...
                if (empty($row['sms_text'])) {
                    ...
                    try {
                        ...
                        mysqli_query($conn, "UPDATE active_number SET active_status='1', status='3' WHERE id='" . (int)$row['id'] . "' AND active_status='2' AND status='2'");
                        refund_once($conn, (int)$user_id, $row['order_id'], $row['service_price'], 'ActiveNumber2');
